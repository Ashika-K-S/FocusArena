...
class Room(models.Model):
    ...
    def check_and_update_status(self):
        ...
        if self.status == "ACTIVE" and self.started_at:
            end_time = self.started_at + timedelta(minutes=self.time_limit_minutes)
...
class Submission(models.Model):
    ...
    language = models.CharField(
        max_length=30, choices=LANGUAGE_CHOICES, default="PYTHON"
    )
...
class ContestChallenge(models.Model):
    room = models.ForeignKey(
        Room, on_delete=models.CASCADE, related_name="contest_challenges"
    )
    challenge = models.ForeignKey(Challenge, on_delete=models.CASCADE)
