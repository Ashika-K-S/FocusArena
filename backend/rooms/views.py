...
from .models import Room, RoomParticipant, Challenge, Submission, ContestChallenge
...
class CreateRoomView(APIView):
    ...
    def post(self, request):
        ...
        return Response(
            {"message": "Room created successfully", "room_code": room.room_code},
            status=201,
        )
        ...
        try:
            ...
            if room.status == "ACTIVE":
                ...
                winner = RoomParticipant.objects.filter(
                    room=room, score__gte=total_possible
                ).first()
                if winner:
                    room.status = "FINISHED"
                    room.ended_at = timezone.now()
...
class StartRoomView(APIView):
    ...
    def post(self, request, room_code):
        ...
        room.save()
        return Response({"message": "Room started successfully", "status": room.status})
class SubmitSolutionView(APIView):
    ...
    def post(self, request, room_code):
        ...
        try:
            contest_problem = room.contest_challenges.select_related("challenge").get(
        ...
        if final_status == "CORRECT":
            ...
            total_possible_points = sum(
                cc.points for cc in room.contest_challenges.all()
            )
        ...
        async_to_sync(channel_layer.group_send)(
            f"room_{room.room_code}", {"type": "leaderboard_update"}
        )
        async_to_sync(channel_layer.group_send)(
...
class SubmissionListView(APIView):
    permission_classes = [IsAuthenticated]
    def get(self, request, room_code):
        submissions = Submission.objects.filter(room__room_code=room_code).order_by(
            "-submitted_at"
        )
        data = []
...
class LeaderboardView(APIView):
    ...
    def get(self, request, room_code):
        ...
        contest_challenges = (
            room.contest_challenges.all().order_by("order").select_related("challenge")
        )
        leaderboard = []
        for participant in participants:
            ...
            for index, contest_problem in enumerate(contest_challenges):
                ...
                if correct_sub:
                    ...
                    if room.started_at:
                        solve_time = (
                            correct_sub.submitted_at - room.started_at
                        ).total_seconds()
...
class TabSwitchWarningView(APIView):
    permission_classes = [IsAuthenticated]
    def post(self, request):
        ...
        participant.save()
        return Response({"warnings": participant.tab_switch_count})
