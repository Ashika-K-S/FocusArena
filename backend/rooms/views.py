...
class CreateRoomView(APIView):
    ...
    def post(self, request):
        ...
        available_challenges = list(
            Challenge.objects.filter(difficulty=room.difficulty).order_by('?')[:3]
        )
...
class JoinRoomView(APIView):
    ...
    def post(self, request):
        ...
        already_joined = RoomParticipant.objects.filter(room=room, user=request.user).exists()
        if already_joined:
...
class RoomDetailView(APIView):
    ...
    def get(self, request, room_code):
        try:
            room = Room.objects.get(room_code=room_code)
            if room.started_at:
                end_time = room.started_at + timezone.timedelta(minutes=room.time_limit_minutes)
            ...
            if room.status == "ACTIVE":
                total_possible = sum(cc.points for cc in room.contest_challenges.all())
                winner = RoomParticipant.objects.filter(room=room, score__gte=total_possible).first()
                if winner:
                    room.status = "FINISHED"
                    room.ended_at = timezone.now()
                    room.save()
                    if not RoomParticipant.objects.filter(room=room, is_winner=True).exists():
...
class StartRoomView(APIView):
    ...
    def post(self, request, room_code):
        ...
        if room.created_by != request.user:
            return Response({"error": "Only creator can start room"}, status=403)

        if room.status == "ACTIVE":
            return Response({"error": "Room already active"}, status=400)
...
class SubmitSolutionView(APIView):
    ...
    def post(self, request, room_code):
        ...
        try:
            contest_problem = room.contest_challenges.select_related("challenge").get(challenge_id=problem_id)
        except ContestChallenge.DoesNotExist:
            return Response({"error": "Problem not found in this room"}, status=404)
        ...
        already_solved = Submission.objects.filter(
            room=room, user=request.user, challenge=challenge, status="CORRECT"
        ).exists()

        if already_solved:
            return Response({"error": "Problem already solved"}, status=400)
        ...
        if timezone.now() > end_time:
            ...
            room.ended_at = timezone.now()
            room.save()
            return Response({"error": "Battle ended"}, status=400)

        test_cases = challenge.test_cases.all()
        execution_result = execute_python_code(code, test_cases)
        final_status = execution_result.get("status", "ERROR")

        submission = Submission.objects.create(
            room=room, user=request.user, challenge=challenge,
            code=code, language=language, status=final_status
        )
        ...
        if final_status == "CORRECT":
            ...
            for cc in room.contest_challenges.all():
                if Submission.objects.filter(room=room, user=request.user, challenge=cc.challenge, status="CORRECT").exists():
...
class SubmissionListView(APIView):
    ...
    def get(self, request, room_code):
        submissions = Submission.objects.filter(room__room_code=room_code).order_by("-submitted_at")
        data = []
...
class LeaderboardView(APIView):
    ...
    def get(self, request, room_code):
        ...
        contest_challenges = room.contest_challenges.all().order_by('order').select_related('challenge')
        leaderboard = []
        ...
        for participant in participants:
            ...
            for index, contest_problem in enumerate(contest_challenges):
                label = chr(65 + index)
                correct_sub = Submission.objects.filter(
                    room=room, user=participant.user, challenge=contest_problem.challenge, status="CORRECT"
                ...
                if correct_sub:
                    ...
                    wrong_attempts = Submission.objects.filter(
                        room=room, user=participant.user, challenge=contest_problem.challenge,
                    ...
                    status = latest.status if latest else "NOT_SOLVED"
            ...
            leaderboard.append({
                ...
                "penalty": int(total_penalty_seconds / 60),
