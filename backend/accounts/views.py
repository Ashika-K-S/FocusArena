...
class RefreshTokenView(APIView):
    ...
    def post(self, request):
        ...
        session = UserSession.objects.filter(refresh_token=refresh, is_active=True).first()
        if not session:
        ...
        try:
            ...
            res.set_cookie(key="access", value=access, httponly=True, secure=False, samesite="Lax", path="/")
            res.set_cookie(key="refresh", value=new_refresh, httponly=True, secure=False, samesite="Lax", path="/")
            return res
        except Exception:
...
class GoogleLoginView(APIView):
    ...
    def post(self, request):
        ...
        res.set_cookie(key="access", value=str(refresh.access_token), httponly=True, secure=False, samesite="Lax", path="/")
        res.set_cookie(key="refresh", value=str(refresh), httponly=True, secure=False, samesite="Lax", path="/")
        return res
