...
const SessionsPage = () => {
  ...
  const getDeviceIcon = (device) => {
    ...
    if (d.includes('iphone') || d.includes('android') || d.includes('mobile')) return <Smartphone size={20} />;
  ...
  return (
    ...
    >
      ...
      <div className="glass-card" style={{ padding: '0' }}>
              ...
              {sessions.map((session, index) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '1.25rem',
                    borderRadius: '1rem',
                    background: session.current ? 'rgba(99, 102, 241, 0.05)' : 'rgba(255, 255, 255, 0.02)',
                    border: session.current ? '1px solid rgba(99, 102, 241, 0.2)' : '1px solid var(--border-color)',
                  }}
                >
                  <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    <div style={{ 
                      background: session.current ? 'var(--primary)' : 'var(--bg-surface)', 
                      padding: '0.75rem', 
                    ...
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <p style={{ fontWeight: '600' }}>{session.device || "Unknown Device"}</p>
                        {session.current && (
                          <span style={{ 
                            fontSize: '0.625rem', 
                  ...
                  <div>
                    {session.current ? (
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--success)', fontSize: '0.75rem', fontWeight: '600' }}>
        ...
        <div style={{ padding: '1.5rem', borderTop: '1px solid var(--border-color)', background: 'rgba(239, 68, 68, 0.02)' }}>
          <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
            <div style={{ color: 'var(--error)', marginTop: '0.25rem' }}>
              <AlertTriangle size={20} />
            </div>
            <div>
              <p style={{ fontWeight: '600', fontSize: '0.875rem' }}>Danger Zone</p>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.8125rem' }}>
                If you suspect unauthorized access, log out from all devices immediately and update your password.
              </p>
            </div>
          </div>
          <button
